...
#define SM2135_CHECK(a, str, ret)  if(!(a)) {                                      \
...
/**
 * SM2135 register address
 */
#define REG_MAX_CURR_CTL 0xC0
#define REG_STANDBY      0xC1
#define REG_OUT1         0xC2
#define REG_OUT2         0xC3
#define REG_OUT3         0xC4
#define REG_OUT4         0xC5
#define REG_OUT5         0xC6
...
/**
 * @brief The chip SM2135EH doesn't support a real I2C interface, it's similar
 *
 */
static esp_err_t sm2135_reg_write(uint8_t addr, uint8_t *data_wr, size_t size)
{
...
static esp_err_t sm2135_set_standby(bool isenable)
{
    uint8_t reg_temp = 0;
    reg_temp = isenable ? (1 << 5) : 0;
    return sm2135_reg_write(REG_STANDBY, &reg_temp, 1);
}
static esp_err_t sm2135_init(i2c_port_t i2c_master_port, gpio_num_t sda_io_num, gpio_num_t scl_io_num, uint32_t clk_freq_hz)
{
    esp_err_t ret = ESP_OK;
    g_i2c_master_num = i2c_master_port;
    ret = i2c_master_init(sda_io_num, scl_io_num, clk_freq_hz);
    SM2135_CHECK(ESP_OK == ret, "i2c init failed", ESP_FAIL);
    ret = sm2135_set_standby(0);
    SM2135_CHECK(ESP_OK == ret, "sm2135 set standby failed", ESP_FAIL);
    return ESP_OK;
}

static esp_err_t sm2135_deinit(void)
{
    sm2135_set_standby(1);
    i2c_driver_delete(g_i2c_master_num);
    return ESP_OK;
}

static esp_err_t sm2135_set_max_current(rgb_curr_t rgb_curr, wy_curr_t wy_curr)
{
    SM2135_CHECK(rgb_curr < RGB_CURR_MAX, "paramter invalid", ESP_ERR_INVALID_ARG);
    SM2135_CHECK(wy_curr < WY_CURR_MAX, "paramter invalid", ESP_ERR_INVALID_ARG);
    uint8_t reg_temp = 0;
    reg_temp = (rgb_curr << 4) | (wy_curr);
    return sm2135_reg_write(REG_MAX_CURR_CTL, &reg_temp, 1);
}

static esp_err_t sm2135_set_channel(uint8_t channel, uint8_t value)
{
    if (_CHANNEL_INVALID == channel) {
        return ESP_OK;
    }

    SM2135_CHECK(channel < _CHANNEL_MAX, "paramter invalid", ESP_ERR_INVALID_ARG);
    uint8_t reg_temp;
    reg_temp = value;
    return sm2135_reg_write(REG_OUT1 + channel, &reg_temp, 1);
}
    ...
    for (int channel = 0; channel < _CHANNEL_MAX; channel++) {
        ...
        if (fade_data->num > 0) {
            ...
            if (fade_data->step) {
                ...
                sm2135_set_channel(g_ch2gpio[channel], gamma_value_to_duty(fade_data->cur));
            } else {
                sm2135_set_channel(g_ch2gpio[channel], gamma_value_to_duty(fade_data->cur));
            }
...
mdf_err_t iot_led_init(ledc_timer_t timer_num, ledc_mode_t speed_mode, uint32_t freq_hz)
{
    ...
    ret = sm2135_init(I2C_NUM_1, I2C_SDA_GPIO, I2C_SCL_GPIO, I2C_FREQ_HZ);
...
mdf_err_t iot_led_regist_channel(ledc_channel_t channel, gpio_num_t out_num)
{
    mdf_err_t ret = MDF_OK;
